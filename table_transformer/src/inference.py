...
def objects_to_crops(img, tokens=None, objects=None, class_thresholds=None, padding=10):
    """
    ...
    Parameters:
        ...
        tokens: List of token dictionaries with bbox information (optional)
        objects: List of detected object dictionaries
        class_thresholds: Dictionary of class threshold values
        padding: Padding to add around detected tables when cropping
    
    Returns:
        List of dictionaries containing cropped images and optionally processed tokens
    """

    # If no objects provided, return empty list
    if not objects:
        return []

    # If no class thresholds provided, accept all objects
    if class_thresholds is None:
        class_thresholds = {"table": 0.5, "table rotated": 0.5, "no object": 10}

    table_crops = []
    for obj in objects:
        # Skip objects that don't meet the threshold
        if class_thresholds and obj['score'] < class_thresholds[obj['label']]:
            continue
            ...
            if obj['label'] == 'table rotated':
                ...
                for token in table_tokens:
                    ...
                    token['bbox'] = [cropped_img.size[0]-token_bbox[3]-1,
                            ...
                            token_bbox[2]]
        # For case without tokens, just rotate the image if needed
        elif obj['label'] == 'table rotated':
            cropped_img = cropped_img.rotate(270, expand=True)
...
def visualize_detected_tables(img, det_tables, out_path, cmap="gray"):
    """
    Visualiza las tablas detectadas sobre una imagen.
...
def visualize_cells(img, cells, out_path, cmap="gray"):
    """
    Visualiza las celdas de tabla sobre una imagen.
    
    Parameters:
        img: PIL Image object
        cells: Lista de celdas detectadas
        out_path: Ruta donde guardar la visualización
        cmap: Mapa de colores para la imagen (default: "gray" para escala de grises)
    """
    # Convertir a array numpy si es una imagen PIL
...
class TableExtractionPipeline(object):
    ...
    def _convert_grayscale_to_rgb(self, img):
    ...
    def detect(self, img, tokens=None, out_objects=True, out_crops=False, crop_padding=10):
        ...
        Parameters:
            img: PIL Image object
            tokens: List of token dictionaries with bbox information (optional)
            out_objects: Whether to include detected objects in the output
            out_crops: Whether to include cropped table images in the output
            crop_padding: Padding to add around detected tables when cropping
            
        Returns:
            Dictionary with detected objects and/or cropped tables
        """
        out_formats = {}
        ...
        img = self._convert_grayscale_to_rgb(img)
